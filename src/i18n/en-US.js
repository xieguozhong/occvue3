export default {
    ...
    , title: {
        ACPI: {
            title: 'ACPI (Advanced Configuration and Power Interface) is an open standard for discovering and configuring computer hardware. ACPI specification defines standard tables (such as DSDT, SSDT, FACS, DMAR) and various implementation methods (such as _DSM, _PRW). Modern hardware requires few changes to maintain ACPI compatibility, but some of the hardware is provided as part of OpenCore. ',
            Add: {
                Title: 'Type: plist array  nFailsafe: empty  nNote: Load the selected table from OC/ACPI directory. \n  n1. Comment  nType: plist string  nFailsafe: Empty string  nNote: Any ASCII string used to provide human readable references for entries. Whether to use this value is implemented. \n2. Enabled  nType: List Boolean  nFailsafe: Error  nNote: This ACPI table will not be added unless set to true. \n3. Path  n Type: plist string  n Failsafe: empty string  n Description: The file path to be loaded as an ACPI table. Sample values include DSDT.aml, SubDir/SSDT-8.aml, SSDT-USBX Ml. '
            },
            Delete: {
                Title: 'Masking ACPI (DSDT, SSDT) tables, (this item can be deleted, but most users cannot use it)'
            },
            Patch: {
                Title: 'Find and replace the contents of the DSDT (SSDT)'
            },
            Quirks: {
                Title: 'ACPI related settings',
                FadtEnableReset: 'NO Repair the restart and shutdown on the old hardware. It is not recommended to enable it unless necessary',
                NormalizeHeaders: 'NO Clear ACPI header field, only required for macOS 10.13',
                RebaseRegions: 'NO Attempts to tentatively relocate the ACPI memory region. Unless a custom DSDT is used, it is not required',
                ResetHwSig: 'NO The hardware that wakes up from hibernation due to failure to maintain the hardware signature after restart needs to be enabled',
                ResetLogoStatus: 'NO The hardware that cannot display the OEM Windows flag on the system with the BGRT table needs to be turned on',
                SyncTableIds: 'NO Synchronize table identifier with SLIC table',
                EnableForAll: 'NO for all systems (exclusive for OpenCore Mod)'
            }
        ...
        PlatformInfo: {
            ...
            Generic: {
                ...
                MLB: '主板序列号，用 Hackintool 读取或生成',
                ProcessorType: '0 处理器主要类型和次要类型的组合',
                ROM: '可以是任意 6 Byte MAC 地址, 如 0x112233000000',
                SystemProductName: '型号标识符，根据实际硬件情况自行选取',
                SystemSerialNumber: '序列号，用 Hackintool 读取或生成',
