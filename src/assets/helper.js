import { fandou, copyDatatoClipboard, showTipModal,stringToJSON,getMaxrowid } from './comm.js'
export default class Helper {
    ...
    **/
    resetSubGridTable(pid, gridkey, theData, keyname) {
        ...
        let rightName = keyname + 'Right', objGridTable = this.table.getTable(gridkey);
        //objGridTable.jqGrid('resetSelection');

        for (let i = 0, len = theData[rightName].length; i < len; i++) {
            if (theData[rightName][i].pid == pid) { //注意：这里一定是==,不能===
    ...
    //勾选页面点击确定按钮事件
     checkboxPageBtnOKclick () {
        ...
        if(this.popdata.RADIO_CHECK_BOX === 'C') {
            ...
            this.base[this.popdata.last_radiobox_ids[1]][this.popdata.last_radiobox_ids[2]][this.popdata.last_radiobox_ids[3]] = this.popdata.pageRadio_CurrentValue;
        }
